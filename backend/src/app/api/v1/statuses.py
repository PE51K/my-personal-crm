...
async def list_statuses(
    ...
    # Get contact counts for all statuses in a single query
    status_ids = [status_obj.id for status_obj in statuses_data]
    count_query = (
        select(Contact.status_id, func.count(Contact.id))
        .where(Contact.status_id.in_(status_ids))
        .group_by(Contact.status_id)
    )
    count_result = await db.execute(count_query)
    contact_counts = dict(count_result.fetchall())

    # Build response with contact counts
    statuses = []
    for status_obj in statuses_data:
        contact_count = contact_counts.get(status_obj.id, 0)
...
async def update_status(
    ...
    count_result = await db.execute(
        select(func.count(Contact.id)).where(Contact.status_id == status_obj.id)
    )
